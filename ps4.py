...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        allRsquared.append(r_squared(y, estimated))
        
    bestFit = max(allRsquared)
    index = allRsquared.index(bestFit)
    estimated = np.polyval(models[index], x)
    
    pylab.plot(x, y, 'bo')
    pylab.plot(x, estimated, 'r')
    pylab.xlabel('Years')
    pylab.ylabel('Avg. Temperature')
    pylab.title('r**2 = ' + str(bestFit))
